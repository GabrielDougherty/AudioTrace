...
[x] Make sure that there's an app name (ideally window title name) in the list
    - now shows "App Name: Window Title" when window title is available
[x] fix the warning in the build:
```
...
[x] Applications launched after launching AudioTrace are not monitored
[ ] Update permission flow to tell user exactly how to enable permissions
